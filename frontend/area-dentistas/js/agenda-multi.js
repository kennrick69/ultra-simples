...
async function carregarPacientes() {
    try {
        // Carregar todos os pacientes para busca (sem paginação)
        const data = await apiGet('/api/pacientes?limit=10000');
...
async function renderizarHorarios(coluna) {
    ...
    horarios.forEach(hora => {
        ...
        if (agendamento) {
            ...
            const telefone = agendamento.paciente_telefone || '';
            const temTelefone = telefone && telefone !== '-';
            ...
            html += `<div class="slot busy ${agendamento.encaixe ? 'encaixe' : ''} ${statusClass}" onclick="editarAgendamento(${coluna}, ${agendamento.id})">
                ...
                <div class="slot-actions">
                    ${temTelefone ? `<button class="slot-whatsapp" onclick="event.stopPropagation(); abrirWhatsAppSlot('${telefone}', '${(agendamento.paciente_nome || 'Paciente').replace(/'/g, "\\'")}', '${hora}')" title="WhatsApp"><svg viewBox="0 0 24 24" width="14" height="14" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></button>` : ''}
                    <span class="slot-dur">${durMin}min</span>
                </div>
            </div>`;
...
var buscaPacienteTimeout = null;

async function buscarPacienteAgendamento() {
    ...
    buscaPacienteTimeout = setTimeout(async function() {
        // Mostrar loading
        container.innerHTML = '<div style="padding: 15px; color: #999;">🔍 Buscando...</div>';
        ...
        // Buscar no servidor
        try {
            const data = await apiGet('/api/pacientes?busca=' + encodeURIComponent(termo) + '&limit=10');
            const filtrados = (data && data.pacientes) ? data.pacientes : [];
            ...
            container.innerHTML = '<div style="padding: 15px; color: #c00;">Erro ao buscar</div>';
        }
    }, 300);
}
...
function editarPacienteAgendamento() {
    const pacienteId = document.getElementById('agendamentoPacienteId').value;
    ...
    // Usar o modal de paciente em modo edição
    if (typeof ModalPaciente !== 'undefined' && ModalPaciente.editar) {
        ModalPaciente.editar(pacienteId, function(pacienteAtualizado) {
            // Callback quando salvar - atualizar dados na tela
            if (pacienteAtualizado) {
                document.getElementById('agendamentoPacienteNome').textContent = pacienteAtualizado.nome;
                document.getElementById('agendamentoPacienteTel').textContent = pacienteAtualizado.celular || pacienteAtualizado.telefone || '-';
        ...
        window.open('pacientes.html?edit=' + pacienteId, '_blank');
    }
}
...
let _orcPendentesList = [];
let _orcPendentesVencidosCount = 0;
let orcPendentesDias = parseInt(localStorage.getItem('orc_pendentes_dias') || '90');
...
async function carregarTodasNotificacoes() {
    ...
    await Promise.all([
        ...
        carregarRecados(),
        carregarPrazosProteticos(),
        carregarRetornosPendentes(),
        carregarOrcPendentes()
    ]);
...
async function carregarPrazosProteticos() {
    try {
        const response = await apiGet('/api/casos-proteticos');
        const casos = (response && response.casos) ? response.casos : [];
        
        // Filtrar casos com prazo em até 2 dias ou atrasados
        const hoje = new Date();
        hoje.setHours(12, 0, 0, 0);
        const doisDias = new Date(hoje);
        doisDias.setDate(doisDias.getDate() + 2);
        
        notifPrazos = casos.filter(c => {
            if (c.status === 'finalizado' || c.status === 'cancelado' || c.status === 'recebido_clinica') return false;
            if (!c.dataPrometida) return false;
            const prazo = parseDataLocalPrazo(c.dataPrometida);
            if (!prazo) return false;
            return prazo <= doisDias;
        });
        
        // Ordenar por prazo (mais urgentes primeiro)
        notifPrazos.sort((a, b) => {
            const pa = parseDataLocalPrazo(a.dataPrometida);
            const pb = parseDataLocalPrazo(b.dataPrometida);
            return pa - pb;
        });
        
        renderizarPrazos();
        document.getElementById('countPrazos').textContent = notifPrazos.length;
        
    } catch (error) {
        console.error('Erro carregar prazos protéticos:', error);
        notifPrazos = [];
        renderizarPrazos();
    }
}

function parseDataLocalPrazo(dataStr) {
    if (!dataStr) return null;
    const str = String(dataStr).split('T')[0];
...
function renderizarPrazos() {
    const container = document.getElementById('listaPrazos');
    
    if (notifPrazos.length === 0) {
        container.innerHTML = '<div class="notif-vazio"><div class="notif-vazio-icon">✅</div><div>Nenhum caso com prazo próximo!</div></div>';
        return;
    }
    ...
    notifPrazos.forEach((caso, i) => {
        const prazo = parseDataLocalPrazo(caso.dataPrometida);
        const diffDias = Math.ceil((prazo - hoje) / (1000 * 60 * 60 * 24));
        
        let statusClass, statusText, statusIcon;
        if (diffDias < 0) {
            statusClass = 'style="border-left: 4px solid #ef4444; background: rgba(239,68,68,0.1);"';
            statusText = `ATRASADO ${Math.abs(diffDias)} dia(s)`;
            statusIcon = '🔴';
        } else if (diffDias === 0) {
            statusClass = 'style="border-left: 4px solid #f59e0b; background: rgba(245,158,11,0.1);"';
            statusText = 'Prazo HOJE';
            statusIcon = '⏰';
        } else {
            statusClass = 'style="border-left: 4px solid #3b82f6; background: rgba(59,130,246,0.1);"';
            statusText = `Faltam ${diffDias} dia(s)`;
            statusIcon = '⚠️';
        }
        
        const prazoFmt = prazo.toLocaleDateString('pt-BR');
        const temTelLab = caso.laboratorioWhatsapp || caso.laboratorioTelefone;
        
        // Pegar nome do procedimento
        let servico = caso.tipoTrabalhoDetalhe || caso.tipoTrabalho || '-';
        if (servico === 'coroa') servico = 'Coroa';
        else if (servico === 'ponte') servico = 'Ponte';
        else if (servico === 'faceta') servico = 'Faceta';
        else if (servico === 'protese_total') servico = 'Prótese Total';
        else if (servico === 'outro') servico = caso.tipoTrabalhoDetalhe || 'Outro';
        
        html += `
            <div class="notif-item" ${statusClass}>
                <div class="notif-info" style="flex: 1;">
                    <div class="notif-nome">${statusIcon} ${statusText} - ${caso.pacienteNome || '-'}</div>
                    <div class="notif-detalhes">
                        <span>${servico}</span>
                        <span>Lab: ${caso.laboratorioNome || '-'}</span>
                    </div>
                    <div class="notif-detalhes">
                        <span>Prazo: ${prazoFmt}</span>
                        <span>Código: ${caso.codigo || '-'}</span>
                    </div>
                </div>
                ${temTelLab ? `<button class="notif-btn" onclick="cobrarLabNotif(${i})" title="Cobrar laboratório"><svg viewBox="0 0 24 24" width="22" height="22" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></button>` : '<span style="width:40px;height:40px;display:flex;align-items:center;justify-content:center;color:#999;font-size:11px;">Sem tel</span>'}
            </div>`;
    });
...
function cobrarLabNotif(idx) {
    const caso = notifPrazos[idx];
    if (!caso) return;
    
    const telefone = caso.laboratorioWhatsapp || caso.laboratorioTelefone;
    if (!telefone) {
        alert('Laboratório sem telefone cadastrado');
        return;
    }
    ...
    let situacao;
    if (diffDias < 0) {
        situacao = 'O prazo venceu há ' + Math.abs(diffDias) + (Math.abs(diffDias) === 1 ? ' dia' : ' dias');
    } else if (diffDias === 0) {
        situacao = 'O prazo é HOJE';
    } else {
        situacao = 'Faltam ' + diffDias + (diffDias === 1 ? ' dia' : ' dias') + ' para o prazo (' + prazo.toLocaleDateString('pt-BR') + ')';
    }
    
    let clinica = 'nossa clínica';
    try {
        const config = JSON.parse(localStorage.getItem('configClinica') || '{}');
        clinica = config.nome || config.nome_clinica || clinica;
    } catch(e) {}
    
    let servico = caso.tipoTrabalhoDetalhe || caso.tipoTrabalho || '-';
    if (servico.toLowerCase().startsWith('outro')) servico = caso.tipoTrabalhoDetalhe || servico;
    
    const msg = 'Olá ' + (caso.laboratorioNome || 'Laboratório') + '!\n\n' +
        'Aqui é da *' + clinica + '*.\n\n' +
        'Gostaria de saber sobre o andamento do trabalho:\n\n' +
        '*Paciente:* ' + (caso.pacienteNome || '-') + '\n' +
        '*Serviço:* ' + servico + '\n' +
        '*Código:* ' + (caso.codigo || '-') + '\n\n' +
        situacao + '.\n\n' +
        'Poderia me dar uma posição? Obrigado!';
...
function atualizarBadgeNotificacoes() {
    const total = notifConfirmacoes.length + notifAniversarios.length + notifRecados.length + notifPrazos.length + _orcPendentesVencidosCount;
    const badge = document.getElementById('badgeNotificacoesCount');
    if (badge) badge.textContent = total;
    // Retornos badge separado
    const retBadge = document.getElementById('badgeRetornosCount');
    if (retBadge && typeof _retornosVencidosCount !== 'undefined') {
        retBadge.textContent = _retornosVencidosCount;
        retBadge.style.display = _retornosVencidosCount > 0 ? '' : 'none';
    }
}
...
// ==================== WHATSAPP RÁPIDO NO SLOT ====================
function abrirWhatsAppSlot(telefone, nomePaciente, horario) {
    if (!telefone) {
        alert('⚠️ Paciente não possui telefone cadastrado');
        return;
    }
    ...
    try {
        const config = JSON.parse(localStorage.getItem('configClinica') || '{}');
        clinica = config.nome_clinica || config.nomeClinica || config.nome || clinica;
    } catch(e) {}
...
// Carregar ao iniciar
// ========== RETORNOS / ACOMPANHAMENTO ==========
let _retornosList = [];
let _retornosVencidosCount = 0;

function abrirRetornos() {
    document.getElementById('modalNotificacoes').classList.add('open');
    trocarAbaNotif('retornos');
    carregarRetornosPendentes();
}

async function carregarRetornosPendentes() {
    try {
        const response = await apiGet('/api/retornos');
        _retornosList = (response && response.retornos) ? response.retornos.filter(r => r.status === 'pendente') : [];
        
        const hoje = new Date(); hoje.setHours(0,0,0,0);
        let vencidos = 0, proximos = 0, emDia = 0;
        _retornosList.forEach(r => {
            const prox = new Date(r.proximoRetorno); prox.setHours(0,0,0,0);
            const diff = Math.floor((prox - hoje) / 86400000);
            if (diff < 0) { r._class = 'vencido'; vencidos++; }
            else if (diff <= 30) { r._class = 'proximo'; proximos++; }
            else { r._class = 'ok'; emDia++; }
            r._dias = diff;
        });
        
        _retornosVencidosCount = vencidos;
        
        // Sort: vencidos first (most overdue), then proximos, then ok
        _retornosList.sort((a, b) => a._dias - b._dias);
        
        // Update stats
        const elV = document.getElementById('retVencidos'); if (elV) elV.textContent = vencidos;
        const elP = document.getElementById('retProximos'); if (elP) elP.textContent = proximos;
        const elE = document.getElementById('retEmDia'); if (elE) elE.textContent = emDia;
        const elC = document.getElementById('countRetornos'); if (elC) elC.textContent = vencidos + proximos;
        
        renderizarRetornos();
        atualizarBadgeNotificacoes();
    } catch (error) {
        console.error('Erro carregar retornos:', error);
        _retornosList = [];
        renderizarRetornos();
    }
}
...
function renderizarRetornos() {
    const container = document.getElementById('listaRetornos');
    if (!container) return;
    
    if (_retornosList.length === 0) {
        container.innerHTML = '<div class="notif-vazio"><div class="notif-vazio-icon">✅</div><div>Nenhum retorno pendente!</div></div>';
        return;
    }
    
    let html = '';
    _retornosList.forEach((r, i) => {
        const dataStr = new Date(r.proximoRetorno).toLocaleDateString('pt-BR');
        let statusText = '';
        let icon = '🔄';
        if (r._class === 'vencido') {
            statusText = Math.abs(r._dias) + ' dia(s) atrasado';
            icon = '🚨';
        } else if (r._class === 'proximo') {
            statusText = r._dias === 0 ? 'HOJE' : 'em ' + r._dias + ' dia(s)';
            icon = '⚠️';
        } else {
            statusText = 'em ' + r._dias + ' dias';
            icon = '✅';
        }
        
        html += `<div class="ret-item-agenda ${r._class}">
            <div class="ret-icon-ag ${r._class}">${icon}</div>
            <div class="ret-info-ag">
                <div class="ret-nome-ag">${r.pacienteNome || 'Paciente'}</div>
                <div class="ret-detalhe-ag">${r.motivo}${r.dente ? ' · Dente ' + r.dente : ''}${r.procedimentoOrigem ? ' · ' + r.procedimentoOrigem : ''}</div>
            </div>
            <div class="ret-data-ag ${r._class}">
                ${dataStr}<br><span style="font-size:10px;font-weight:400;">${statusText}</span>
            </div>
            <div class="ret-acoes-ag">
                ${r.pacienteCelular ? '<button class="ret-btn-ag wpp" onclick="enviarWhatsRetorno(' + i + ')" title="WhatsApp">📱</button>' : ''}
                <button class="ret-btn-ag verde" onclick="marcarRetornoRealizado(${r.id})" title="Realizado">✅</button>
                <button class="ret-btn-ag red" onclick="excluirRetorno(${r.id})" title="Excluir">🗑️</button>
            </div>
        </div>`;
    });
    container.innerHTML = html;
...
function enviarWhatsRetorno(idx) {
    const r = _retornosList[idx];
    if (!r || !r.pacienteCelular) return;
    const cel = r.pacienteCelular.replace(/\D/g, '');
    const dataStr = new Date(r.proximoRetorno).toLocaleDateString('pt-BR');
    let msg = `Olá ${r.pacienteNome}! 😊\n\n`;
    msg += `Estamos entrando em contato para lembrar que está na hora do seu retorno odontológico.\n\n`;
    msg += `📋 *Motivo:* ${r.motivo}\n`;
    if (r.dente) msg += `🦷 *Dente:* ${r.dente}\n`;
    msg += `📅 *Data prevista:* ${dataStr}\n\n`;
    msg += `Podemos agendar um horário para você? Responda esta mensagem que verificamos a disponibilidade! 🙂`;
    window.open('https://wa.me/55' + cel + '?text=' + encodeURIComponent(msg), '_blank');
}

async function marcarRetornoRealizado(id) {
    if (!confirm('Marcar retorno como realizado?\n\nDeseja agendar automaticamente o próximo retorno?')) return;
    const renovar = confirm('Agendar próximo retorno automaticamente?\n(baseado na periodicidade configurada)');
    try {
        const response = await fetch(API_BASE_URL + '/api/retornos/' + id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
            body: JSON.stringify({ status: 'realizado', renovar: renovar })
        });
        const data = await response.json();
        if (data.success) {
            mostrarToastAgenda(renovar ? '✅ Retorno realizado! Próximo agendado.' : '✅ Retorno realizado!', '#16a34a');
            carregarRetornosPendentes();
        }
    } catch (e) { console.error('Erro marcar retorno:', e); }
}

async function excluirRetorno(id) {
    if (!confirm('Excluir este retorno permanentemente?')) return;
    try {
        const response = await fetch(API_BASE_URL + '/api/retornos/' + id, {
            method: 'DELETE',
        ...
        const data = await response.json();
        if (data.success) {
            mostrarToastAgenda('Retorno removido', '#ef4444');
            carregarRetornosPendentes();
        }
    } catch (e) { console.error('Erro excluir retorno:', e); }
}

function mostrarFormNovoRetorno() {
    const container = document.getElementById('listaRetornos');
    if (!container) return;
    
    // Default: 6 months from now
    const proxData = new Date();
    proxData.setMonth(proxData.getMonth() + 6);
    ...
    container.innerHTML = `
        <div style="padding:12px;background:#f8fafc;border-radius:12px;border:1px solid #e5e7eb;">
            <h4 style="margin:0 0 12px;font-size:14px;color:#7c3aed;">➕ Agendar Novo Retorno</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <div style="grid-column:1/-1;">
                    <label style="font-size:11px;font-weight:600;color:#374151;">Paciente *</label>
                    <input type="text" id="retPacBusca" placeholder="Digite o nome do paciente..." oninput="buscarPacienteRetorno(this.value)" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;">
                    <div id="retPacResultados" style="max-height:120px;overflow-y:auto;"></div>
                    <input type="hidden" id="retPacId">
                </div>
                <div>
                    <label style="font-size:11px;font-weight:600;color:#374151;">Motivo *</label>
                    <select id="retMotivo" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;">
                        <option value="Raio-X de controle">Raio-X de controle</option>
                        <option value="Proservação endodôntica">Proservação endodôntica</option>
                        <option value="Controle periodontal">Controle periodontal</option>
                        <option value="Revisão de prótese">Revisão de prótese</option>
                        <option value="Controle ortodôntico">Controle ortodôntico</option>
                        <option value="Controle de implante">Controle de implante</option>
                        <option value="Limpeza periódica">Limpeza periódica</option>
                        <option value="Outro">Outro</option>
                    </select>
                </div>
                <div>
                    <label style="font-size:11px;font-weight:600;color:#374151;">Dente</label>
                    <input type="text" id="retDente" placeholder="Ex: 36" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;">
                </div>
                <div>
                    <label style="font-size:11px;font-weight:600;color:#374151;">Periodicidade</label>
                    <select id="retPeriodicidade" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;" onchange="atualizarDataRetorno()">
                ...
                <div>
                    <label style="font-size:11px;font-weight:600;color:#374151;">Próximo retorno *</label>
                    <input type="date" id="retData" value="${proxStr}" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;">
                </div>
                <div>
                    <label style="font-size:11px;font-weight:600;color:#374151;">Procedimento origem</label>
                    <input type="text" id="retProcOrigem" placeholder="Ex: Trat. de canal" style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;">
                </div>
                <div style="grid-column:1/-1;">
                    <label style="font-size:11px;font-weight:600;color:#374151;">Observações</label>
                    <textarea id="retObs" rows="2" placeholder="Anotações..." style="width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:Inter,sans-serif;"></textarea>
                </div>
            </div>
            <div style="display:flex;gap:8px;margin-top:12px;justify-content:flex-end;">
                <button onclick="carregarRetornosPendentes()" style="padding:8px 16px;border:1px solid #d1d5db;background:white;border-radius:8px;font-size:12px;cursor:pointer;font-weight:600;">Cancelar</button>
                <button onclick="salvarNovoRetorno()" style="padding:8px 16px;border:none;background:#7c3aed;color:white;border-radius:8px;font-size:12px;cursor:pointer;font-weight:600;">💾 Salvar Retorno</button>
            </div>
        </div>`;
}

let _retBuscaTimeout = null;
function buscarPacienteRetorno(termo) {
    clearTimeout(_retBuscaTimeout);
    if (termo.length < 2) { document.getElementById('retPacResultados').innerHTML = ''; return; }
    _retBuscaTimeout = setTimeout(async () => {
        try {
            const res = await apiGet('/api/pacientes?busca=' + encodeURIComponent(termo));
            const pacientes = (res && res.pacientes) ? res.pacientes.slice(0, 5) : [];
            const container = document.getElementById('retPacResultados');
            if (pacientes.length === 0) { container.innerHTML = '<div style="padding:6px;font-size:11px;color:#9ca3af;">Nenhum paciente encontrado</div>'; return; }
            container.innerHTML = pacientes.map(p =>
                `<div onclick="selecionarPacienteRetorno(${p.id},'${(p.nome||'').replace(/'/g,"\\'")}')" style="padding:8px 10px;cursor:pointer;border-bottom:1px solid #f1f5f9;font-size:12px;transition:background 0.15s;" onmouseover="this.style.background='#f5f3ff'" onmouseout="this.style.background=''">${p.nome}</div>`
            ).join('');
        } catch(e) {}
    }, 300);
}

function selecionarPacienteRetorno(id, nome) {
    document.getElementById('retPacId').value = id;
    document.getElementById('retPacBusca').value = nome;
    document.getElementById('retPacResultados').innerHTML = '';
}

function atualizarDataRetorno() {
    const meses = parseInt(document.getElementById('retPeriodicidade').value);
    const d = new Date();
    d.setMonth(d.getMonth() + meses);
    document.getElementById('retData').value = d.toISOString().split('T')[0];
}

async function salvarNovoRetorno() {
    const pacienteId = document.getElementById('retPacId').value;
    const motivo = document.getElementById('retMotivo').value;
    const dente = document.getElementById('retDente').value;
    const periodicidade = document.getElementById('retPeriodicidade').value;
    const data = document.getElementById('retData').value;
    const procOrigem = document.getElementById('retProcOrigem').value;
    const obs = document.getElementById('retObs').value;
    
    if (!pacienteId) { alert('Selecione um paciente'); return; }
    if (!data) { alert('Informe a data do próximo retorno'); return; }
    
    try {
        const response = await fetch(API_BASE_URL + '/api/retornos', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
            body: JSON.stringify({
                ...
                motivo: motivo,
                dente: dente || null,
                periodicidadeMeses: parseInt(periodicidade),
                proximoRetorno: data,
                procedimentoOrigem: procOrigem || null,
                observacoes: obs || null
            })
        });
        const result = await response.json();
        if (result.success) {
            mostrarToastAgenda('✅ Retorno agendado!', '#7c3aed');
            carregarRetornosPendentes();
        } else {
...
// ========== ORCAMENTOS PENDENTES (Lembrete 90 dias) ==========

function setOrcPendentesDias(dias) {
    orcPendentesDias = dias;
    localStorage.setItem('orc_pendentes_dias', dias);
    document.querySelectorAll('.orc-dias-btn').forEach(btn => {
        btn.classList.toggle('active', parseInt(btn.dataset.dias) === dias);
    });
}

async function carregarOrcPendentes() {
    try {
        const response = await apiGet('/api/orcamentos-pendentes/lembretes');
        _orcPendentesList = (response && response.lembretes) ? response.lembretes : [];

        const hoje = new Date(); hoje.setHours(0,0,0,0);
        let vencidos = 0;
        _orcPendentesList.forEach(op => {
            const lembrete = new Date(op.proximo_lembrete); lembrete.setHours(0,0,0,0);
            const diff = Math.floor((lembrete - hoje) / 86400000);
            op._vencido = diff <= 0;
            op._dias = diff;
            if (op._vencido) vencidos++;

            // Calcular total dos itens
            const itens = typeof op.itens === 'string' ? JSON.parse(op.itens) : (op.itens || []);
            op._itens = itens;
            op._total = itens.reduce((s, i) => s + (parseFloat(i.valor) || 0), 0);
            op._qtd = itens.length;
        });

        _orcPendentesVencidosCount = vencidos;

        // Sort: vencidos first
        _orcPendentesList.sort((a, b) => a._dias - b._dias);

        // Update stats
        const elV = document.getElementById('orcPendVencidos'); if (elV) elV.textContent = vencidos;
        const elT = document.getElementById('orcPendTotal'); if (elT) elT.textContent = _orcPendentesList.length;
        const elC = document.getElementById('countOrcPendentes'); if (elC) elC.textContent = vencidos;

        renderizarOrcPendentes();
        atualizarBadgeNotificacoes();

        // Init dias buttons
        document.querySelectorAll('.orc-dias-btn').forEach(btn => {
            btn.classList.toggle('active', parseInt(btn.dataset.dias) === orcPendentesDias);
        });
    } catch (error) {
        console.error('Erro carregar orc pendentes:', error);
        _orcPendentesList = [];
        _orcPendentesVencidosCount = 0;
        renderizarOrcPendentes();
    }
}

function renderizarOrcPendentes() {
    const container = document.getElementById('listaOrcPendentes');
    if (!container) return;

    if (_orcPendentesList.length === 0) {
        container.innerHTML = '<div class="notif-vazio"><div class="notif-vazio-icon">✅</div><div>Nenhum orcamento pendente!</div></div>';
        return;
    }

    let html = '';
    _orcPendentesList.forEach((op, i) => {
        const dataStr = new Date(op.proximo_lembrete).toLocaleDateString('pt-BR');
        const icon = op._vencido ? '🚨' : '⏳';
        const statusTxt = op._vencido
            ? (Math.abs(op._dias) === 0 ? 'Vence HOJE' : Math.abs(op._dias) + ' dia(s) atrasado')
            : 'em ' + op._dias + ' dias';

        const procs = op._itens.map(it => it.procedimento).join(', ');
        const procsShort = procs.length > 50 ? procs.substring(0, 47) + '...' : procs;

        const celular = op.whatsapp || op.celular || '';

        html += `<div class="orc-pend-item ${op._vencido ? 'vencido' : ''}">
            <div class="orc-pend-icon">${icon}</div>
            <div class="orc-pend-info">
                <div class="orc-pend-nome">${op.paciente_nome || 'Paciente'}</div>
                <div class="orc-pend-detalhe">${op._qtd} proc. - ${procsShort}</div>
                <div class="orc-pend-detalhe">Lembrete: ${dataStr} <span style="color:${op._vencido ? '#dc2626' : '#d97706'};font-weight:600;">(${statusTxt})</span></div>
            </div>
            <div class="orc-pend-valor">R$ ${op._total.toFixed(2).replace('.', ',')}</div>
            <div class="orc-pend-acoes">
                ${celular ? '<button class="orc-pend-btn wpp" onclick="enviarWhatsOrcPendente(' + i + ')" title="Lembrar via WhatsApp">📱</button>' : ''}
                <button class="orc-pend-btn adiar" onclick="adiarOrcPendente(${op.id})" title="Adiar lembrete">⏰</button>
                <button class="orc-pend-btn descartar" onclick="descartarOrcPendente(${op.id})" title="Descartar">🗑️</button>
            </div>
        </div>`;
    });
    container.innerHTML = html;
...
function enviarWhatsOrcPendente(idx) {
    const op = _orcPendentesList[idx];
    if (!op) return;
    const cel = (op.whatsapp || op.celular || '').replace(/\D/g, '');
    if (!cel) { alert('Paciente sem telefone cadastrado'); return; }

    const procs = op._itens.map(it => {
        const v = parseFloat(it.valor) || 0;
        return '- ' + it.procedimento + (it.dente ? ' (dente ' + it.dente + ')' : '') + ': R$ ' + v.toFixed(2).replace('.', ',');
    }).join('\n');

    let msg = 'Olá ' + (op.paciente_nome || '') + '!\n\n';
    msg += 'Estamos entrando em contato para lembrar que você possui procedimentos odontológicos pendentes que foram identificados na sua avaliação.\n\n';
    msg += 'PROCEDIMENTOS PENDENTES:\n' + procs + '\n\n';
    msg += 'Total: R$ ' + op._total.toFixed(2).replace('.', ',') + '\n\n';
    msg += 'Cuidar da saúde bucal é muito importante! Podemos agendar um horário para você? Responda esta mensagem e verificamos a disponibilidade.\n\n';
    msg += 'Atenciosamente,\n' + getNomeClinica();

    window.open('https://wa.me/55' + cel + '?text=' + encodeURIComponent(msg), '_blank');
}

async function adiarOrcPendente(id) {
    const dias = orcPendentesDias;
    const novaData = new Date(Date.now() + dias * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    try {
        await fetch(API_BASE_URL + '/api/orcamentos-pendentes/' + id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
            body: JSON.stringify({ proximoLembrete: novaData })
        });
        mostrarToastAgenda('Lembrete adiado para ' + dias + ' dias', '#f59e0b');
        carregarOrcPendentes();
    } catch(e) { alert('Erro ao adiar'); }
}

async function descartarOrcPendente(id) {
    if (!confirm('Descartar este orcamento pendente?\n\nO paciente nao sera mais lembrado sobre esses procedimentos.')) return;
    try {
        await fetch(API_BASE_URL + '/api/orcamentos-pendentes/' + id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
        ...
        mostrarToastAgenda('Orcamento descartado', '#dc2626');
        carregarOrcPendentes();
